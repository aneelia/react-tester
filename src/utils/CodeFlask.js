...
CodeFlask.prototype.handleInput = function(
  textarea,
  highlightCode,
  highlightPre
) {
  ...
  textarea.addEventListener('keydown', function(e) {
    ...
    currentLineStart =
      selStartPos - input.value.substr(0, selStartPos).split('\n').pop().length;
    ...
    if (e.keyCode === 9) {
      ...
      if (e.shiftKey) {
        ...
        if (
          inputVal.substring(
            currentLineStart,
            currentLineStart + indentLength
          ) == self.indent
        ) {
          input.value =
            inputVal.substring(0, currentLineStart) +
            inputVal.substring(
              currentLineStart + indentLength,
              input.value.length
            );
